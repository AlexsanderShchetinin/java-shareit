...
class UserControllerTest {
    ...
    void getAll() throws Exception {
        ...
        final int amountUsers = 50;
        List<UserDto> users = makeUsers(amountUsers);
        ...
        mvc.perform(get("/users")
                ...
                .andExpect(jsonPath("$[*].id").value(hasItem(users.get(amountUsers / 2).getId().intValue())))
                .andExpect(jsonPath("$[*].email").value(hasItem(users.get(amountUsers / 3).getEmail())))
                .andExpect(jsonPath("$[*].name").value(hasItem(users.get(amountUsers / 4).getName())))
